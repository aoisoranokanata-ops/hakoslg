...
## 5. 盟主の容姿（1:1、imageKey: avatar_*）✅ 納品済み（2026-08-01・6枚）

**同梱済み**。`art/avatar_*.jpg`（192px JPEG q82・1枚8〜10KB・6枚で53KB）。
振り分けは背景の色相で機械的に確定できた（濃紺228°／深紅3°／白紫256°／若草50°／紅5°／水色202°）。
赤系2枚だけ色相が近いので実物を見て判別した（髭の有無で `m2` と `f2` を分けた）。

---

## 5-2. 女性3枚の描き直し（`avatar_f1` / `f2` / `f3`）✅ 納品済み（2026-08-01）

**差し替え済み**。顔立ちと髪色の指定が両方効き、6枚並べた時に男女がはっきり分かれた。
振り分けは背景の色相が初回どおり（49°／5°／206°）で、機械的に確定できた。

**16pxの円での見分けやすさ（実測）**: 16pxへ縮小し円マスクを掛けた画素で比較した。
似ている順に 壮年の将↔紅蓮の女傑 49.2／白髪の軍師↔静謐の女軍師 65.2／青年武官↔若き女将 101.6。
上位2組はどちらも**赤同士・白同士**で、髪色を変えた効果で初回より開いている。
実用上は同盟欄（38px）で判別できれば足りるので、ここは許容範囲とする。
...
## 6. 期間限定イベントのバナー（imageKey: `event_banner`）✅ 納品済み（2026-08-06・1枚）

**同梱済み**。`art/event_banner.jpg`（192px JPEG q82・16.9KB）。
金の光が噴き上がる楼門＋紅白の幟＋提灯という構図で、**72pxでも「祭の金色」として読める**。
人物を描かせなかったので、3種のイベント名（英傑降臨祭／群雄集結／星辰の宴）の
どれに使っても違和感が出ない。
...
## 7. タイトル画面の背景（imageKey: `title_bg`）✅ 納品済み（2026-08-17・1枚）

**同梱済み**。`art/title_bg.jpg`（941×1672 JPEG q88・182KB）。
灰色の城＋ちぎれた幟＋眼下に広がる大陸＋地平線の金という構図で、
**上下が暗く中央帯に主役が来る**という指定どおりに届いた。

納品は941×1672（9:16ちょうど）で、指定の1080×1920より小さいが**縮小しないでそのまま使った**。
原寸を超えて引き伸ばしても情報は増えないため。**300KB前後の想定に対し182KB**で収まったのは、
暗く滑らかな絵はJPEGがよく効くから。空の暗部の誤差は平均0.63/255で階調飛びは出ていない。

**反映時に1件だけ直した**: **横長画面では小さい文字が読めなかった**。
9:16の絵が上下を大きく切られ、**明るい地平線が文字の高さまで上がってくる**ため。
`#titleArt::after` を濃くすると縦画面で絵が沈むので、
**文字ブロック（`.title-top` / `.title-bottom`）の背後だけ**を落として両立させた。
副題と曲名の色も `--fg-dim`(#9aa5bb) から #cdd6e6 へ上げている。
...
- 同梱は他のアートと同じ `art/` ＋ `python tools/embed_art.py`（音声だけ別系統の `embed_bgm.py`）
